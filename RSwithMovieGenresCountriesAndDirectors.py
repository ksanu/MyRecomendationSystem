...
DFMovieCountries=pd.read_csv(datasetFilesPath+DFMovieCountriesFileName, header=0, delimiter="\t", encoding='iso-8859-1')

DFMovieDirectorsFileName="movie_directors.dat"
DFMovieDirectors=pd.read_csv(datasetFilesPath+DFMovieDirectorsFileName, header=0, delimiter="\t", encoding='iso-8859-1', usecols=['movieID', 'directorID'])
...
DFUserRatedMoviesWithMovieGenresAndCountries=pd.merge(DFUserRatedMoviesWithMovieGenres, DFMovieCountriesPivoted, on='movieID')

#----Directors
DFMovieDirectors['dummyColumn']=1
DFMovieDirectorsPivoted = DFMovieDirectors.pivot_table(index="movieID", columns="directorID", values="dummyColumn")
DFMovieDirectorsPivoted['movieID']=DFMovieDirectorsPivoted.index

DFUserRatedMoviesWithMovieGenresCountriesAndDirectors=pd.merge(DFUserRatedMoviesWithMovieGenresAndCountries, DFMovieDirectorsPivoted, on='movieID')
DFUserRatedMoviesWithMovieGenresCountriesAndDirectors=DFUserRatedMoviesWithMovieGenresCountriesAndDirectors.fillna(value=0)

#preprocessing..
DFUserRatedMoviesWithMovieGenresCountriesAndDirectors["rating"]=DFUserRatedMoviesWithMovieGenresCountriesAndDirectors["rating"]>DFUserRatedMoviesWithMovieGenresCountriesAndDirectors["rating"].mean()
DFUserRatedMoviesWithMovieGenresCountriesAndDirectors_y=DFUserRatedMoviesWithMovieGenresCountriesAndDirectors["rating"]
...
DFUserRatedMoviesWithMovieGenresCountriesAndDirectors_X=DFUserRatedMoviesWithMovieGenresCountriesAndDirectors.drop("rating",1)
...
DFUserRatedMoviesWithMovieGenresCountriesAndDirectors_X=DFUserRatedMoviesWithMovieGenresCountriesAndDirectors_X.drop("userID",1)
DFUserRatedMoviesWithMovieGenresCountriesAndDirectors_X=DFUserRatedMoviesWithMovieGenresCountriesAndDirectors_X.drop("movieID",1)
...
def getXy_MovieGenresCountriesAndDirectors(nrows=1000):
    datasetFilesPath = "C:\\hetrec2011-movielens-2k-v2\\"
    ...
    DFUserRatedMoviesWithMovieGenresAndCountries = pd.merge(DFUserRatedMoviesWithMovieGenres, DFMovieCountriesPivoted,
                                                            on='movieID')
...
def getTrainedLRClassifier(X_train,X_test,y_train,y_test):
    LRC = LogisticRegression()
    LRC.fit(X_train, y_train)
    y_probs = LRC.predict_proba(X_test)
    y_pred = y_probs[:, 1]
